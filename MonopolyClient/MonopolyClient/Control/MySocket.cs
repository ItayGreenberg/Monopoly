            ...
            if (data.Length > 0 && socket.Connected)
            {
                try
                {
                    ...
                    if (bytesSent != buffer.Length)
                    {
                        ...
                        //ClientForm.s_instance.UpdateDebugOutput("Not all bytes were sent.");
                    }
                    else
                    {
                        // All bytes were sent successfully
                        //ClientForm.s_instance.UpdateDebugOutput("Data sent successfully.");
                    }
                }
                catch (SocketException ex)
                {
                    // Handle socket exception
                    //ClientForm.s_instance.UpdateDebugOutput($"SocketException: {ex.Message}");
                }
                catch (Exception ex)
                {
                    // Handle other exceptions
                    //ClientForm.s_instance.UpdateDebugOutput($"Exception: {ex.Message}");
                }
            }
        }
